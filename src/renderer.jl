...
request(m::Tree; kwargs...) = request(terminal, m; kwargs...)

function request(term::REPL.Terminals.TTYTerminal, m::Tree; isedit = false)
    global mem
    mem = Dict{Any, Any}()

    cursor = 0

    printMenu(term.out_stream, m, cursor, init=true, isedit = isedit)
